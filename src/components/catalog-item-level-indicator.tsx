...
export interface CatalogItemLevelIndicatorProps {
	nodeId: TreeNodeEntity['id'];
}

export function CatalogItemLevelIndicator({
	nodeId
}: Readonly<CatalogItemLevelIndicatorProps>) {
	const isDragging = useBookCatalogTree((state) => state.isDragging);
	const currentDropNode = useBookCatalogTree((state) => state.currentDropNode);
	const reachLevel = useBookCatalogTree((state) =>
		state.reachLevelMap.get(nodeId)
	);
	const minReachLevel = useBookCatalogTree(
