import { TreeNodeEntity, DocEntity } from '@prisma/client';

import { TreeNodeView } from '@/types/tree-node';

const getSubTreeMaxLevel = (
	nodeMap: Map<number, TreeNodeView>,
	nodeId: TreeNodeView['id']
) => {
	const node = nodeMap.get(nodeId);

	if (!node) {
		return -1;
	}

	const children = Array.from(nodeMap.values()).filter(
		(node) => node.parentId === nodeId
	);

	let maxLevel = node.level;

	for (const child of children) {
		const childMaxLevel = getSubTreeMaxLevel(nodeMap, child.id);

		if (childMaxLevel > maxLevel) {
			maxLevel = childMaxLevel;
		}
	}

	return maxLevel;
};

export const checkShouldMoveNode = (
	nodeMap: Map<number, TreeNodeView>,
	{
		nodeId,
		newParentId,
		newPrevId
	}: {
		nodeId: TreeNodeView['id'];
		newParentId: TreeNodeView['parentId'];
		newPrevId: TreeNodeView['prevId'];
	}
) => {
	const node = nodeMap.get(nodeId);

	return {
		shouldUpdateNode:
			node?.parentId !== newParentId || node?.prevId !== newPrevId,
		node,
		subTreeMaxLevel: getSubTreeMaxLevel(nodeMap, nodeId)
	};
};

const removeNodeFromOldPosition = (
	nodeMap: Map<number, TreeNodeView>,
	node: TreeNodeView
) => {
	if (node.parentId === node.prevId) {
		const parentNode = node.parentId ? nodeMap.get(node.parentId) : null;

		if (parentNode) {
			parentNode.childId = node.siblingId;
		}

		const siblingNode = node.siblingId ? nodeMap.get(node.siblingId) : null;

		if (siblingNode) {
			siblingNode.prevId = node.prevId;
		}

		return;
	}

	const prevNode = node.prevId ? nodeMap.get(node.prevId) : null;

	if (prevNode) {
		prevNode.siblingId = node.siblingId;
	}

	const siblingNode = node.siblingId ? nodeMap.get(node.siblingId) : null;

	if (siblingNode) {
		siblingNode.prevId = node.prevId;
	}
};

export const deleteNode = (
	nodeMap: Map<number, TreeNodeView>,
	nodeId: TreeNodeView['id']
) => {
	const nodeIdsToDelete: number[] = [];
	const docIdsToDelete: DocEntity['id'][] = [];

	const deletedNode = nodeMap.get(nodeId);

	nodeMap.delete(nodeId);

	if (!deletedNode) {
		return [nodeIdsToDelete, docIdsToDelete];
	}

	nodeIdsToDelete.push(nodeId);

	if (deletedNode.docId) {
		docIdsToDelete.push(deletedNode.docId);
	}

	removeNodeFromOldPosition(nodeMap, deletedNode);

	const deleteChildren = () => {
		const collectNodeIds = (parentId: number) => {
			for (const node of nodeMap.values()) {
				if (node.parentId === parentId) {
					nodeIdsToDelete.push(node.id);

					if (node.docId) {
						docIdsToDelete.push(node.docId);
					}

					collectNodeIds(node.id);
				}
			}
		};

		collectNodeIds(nodeId);

		for (const id of nodeIdsToDelete) {
			nodeMap.delete(id);
		}
	};

	deleteChildren();

	return [nodeIdsToDelete, docIdsToDelete];
};

const prependChild = (
	nodeMap: Map<number, TreeNodeView>,
	{
		node,
		newParentId
	}: {
		node: TreeNodeView;
		newParentId: TreeNodeView['parentId'];
		newPrevId: TreeNodeView['prevId'];
	}
) => {
	if (newParentId) {
		const newParentNode = nodeMap.get(newParentId);

		if (!newParentNode) {
			return;
		}

		const newParentNodeChild = newParentNode.childId
			? nodeMap.get(newParentNode.childId)
			: null;

		newParentNode.childId = node.id;

		node.parentId = newParentId;
		node.prevId = newParentId;
		node.siblingId = newParentNodeChild ? newParentNodeChild.id : null;
		node.level = newParentNode.level + 1;

		if (newParentNodeChild) {
			newParentNodeChild.prevId = node.id;
		}
	} else {
		const firstChildOfRoot = Array.from(nodeMap.values()).find(
			(node) => node.parentId === null && node.prevId === null
		);

		node.parentId = null;
		node.prevId = null;
		node.siblingId = firstChildOfRoot ? firstChildOfRoot.id : null;
		node.level = 0;

		if (firstChildOfRoot) {
			firstChildOfRoot.prevId = node.id;
		}
	}
};

const moveAfter = (
	nodeMap: Map<number, TreeNodeView>,
	{
		node,
		newPrevId
	}: {
		node: TreeNodeView;
		newParentId: TreeNodeView['parentId'];
		newPrevId: TreeNodeView['prevId'];
	}
) => {
	const newPrevNode = newPrevId ? nodeMap.get(newPrevId) : null;

	if (!newPrevNode) {
		return;
	}

	const newPrevNodeSibling = newPrevNode.siblingId
		? nodeMap.get(newPrevNode.siblingId)
		: null;

	newPrevNode.siblingId = node.id;

	node.parentId = newPrevNode.parentId;
	node.prevId = newPrevNode.id;
	node.siblingId = newPrevNodeSibling ? newPrevNodeSibling.id : null;
	node.level = newPrevNode.level;

	if (newPrevNodeSibling) {
		newPrevNodeSibling.prevId = node.id;
	}
};

const insertNodeIntoNewPosition = (
	nodeMap: Map<number, TreeNodeView>,
	{
		node,
		newParentId,
		newPrevId
	}: {
		node: TreeNodeView;
		newParentId: TreeNodeView['parentId'];
		newPrevId: TreeNodeView['prevId'];
	}
) => {
	if (newParentId === newPrevId) {
		prependChild(nodeMap, {
			node,
			newParentId,
			newPrevId
		});
	} else {
		moveAfter(nodeMap, {
			node,
			newParentId,
			newPrevId
		});
	}
};

export const moveNode = (
	nodeMap: Map<number, TreeNodeView>,
	{
		node,
		newParentId,
		newPrevId
	}: {
		node: TreeNodeView;
		newParentId: TreeNodeView['parentId'];
		newPrevId: TreeNodeView['prevId'];
	}
) => {
	removeNodeFromOldPosition(nodeMap, node);
	insertNodeIntoNewPosition(nodeMap, {
		node,
		newParentId,
		newPrevId
	});
};

export const publishNode = (
	nodeMap: Map<number, TreeNodeView>,
	nodeId: TreeNodeView['id']
) => {
	const nodeIdsToPublish: TreeNodeEntity['id'][] = [];
	const docIdsToPublish: DocEntity['id'][] = [];

	const publish = (nodeId: number) => {
		const node = nodeMap.get(nodeId);

		if (!node || node.isPublished) {
			return;
		}

		node.isPublished = true;
		nodeIdsToPublish.push(nodeId);

		if (node.docId) {
			docIdsToPublish.push(node.docId);
		}

		if (node.parentId) {
			publish(node.parentId);
		}
	};

	publish(nodeId);

	return [nodeIdsToPublish, docIdsToPublish];
};

export const unpublishNode = (
	nodeMap: Map<number, TreeNodeView>,
	nodeId: TreeNodeView['id']
) => {
	const nodeIdsToUnpublish: TreeNodeEntity['id'][] = [];
	const docIdsToUnpublish: DocEntity['id'][] = [];

	const unpublish = (nodeId: number) => {
		const node = nodeMap.get(nodeId);

		if (!node || !node.isPublished) {
			return;
		}

		node.isPublished = false;
		nodeIdsToUnpublish.push(nodeId);

		if (node.docId) {
			docIdsToUnpublish.push(node.docId);
		}

		const children = nodeMap
			.values()
			.filter((node) => node.parentId === nodeId);

		for (const child of children) {
			unpublish(child.id);
		}
	};

	unpublish(nodeId);

	return [nodeIdsToUnpublish, docIdsToUnpublish];
};
